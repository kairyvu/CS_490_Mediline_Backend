...
@token_required
def get_all_upcoming_appointments(user: tuple[int, User], user_id):
    _user_id, _user = user
...
#@swag_from('../docs/appointment_routes/get_all_upcoming_appointments.yml')
@token_required
def get_all_upcoming_appointments_auth(user):
    user_id, _ = user
    sort_by = request.args.get('sort_by', 'start_date')
    order = request.args.get('order', 'desc')
...
#@swag_from('../docs/appointment_routes/create_appointment.yml')
@token_required
def create_appointment_auth(user):
    user_id, user = user
    if user.account_type.name != 'Doctor':
        return jsonify({"error": "only doctors can create appointments"}), 401
...
#@swag_from('../docs/appointment_routes/update_appointment_detail.yml')
@token_required
def update_appointment_detail_auth(user, appointment_id):
    data = request.get_json()
    if not data:
        return jsonify({"error": "No input data provided"}), 400
    treatment = data.get('treatment')
...
#@swag_from('../docs/appointment_routes/get_appointment_by_id.yml')
@token_required
def get_appointment_by_id_auth(user, appointment_id):
    try:
        appointment = get_appointment(appointment_id)
        return jsonify(appointment), 200
    except ValueError as e:
